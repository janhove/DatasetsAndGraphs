...
Use a logical structure in your scripts (e.g., a new section for each exercise if you're working with
the `script_with_markdown.R` template).
Compile the R script as a HTML file. This will only work if your R code is 
syntactically well-formed and self-contained.
Check if the HTML file in fact shows the correct results.
If not, this suggests that the results you'd originally obtained
depend on some objects that are not defined in the script (or that are defined
differently in the script from how you think they are defined).
Hand in both the R file as well as the HTML file.
Please use one script for all the exercises related to the datasets lecture
and another script for all the exercises related to the data visualisation lecture.
:::
...
of your script. Remove any `library()` commands for packages you ended up 
not using in the script.

::: {.callout-tip}
## Documenting software versions
Both R itself as well as R packages evolve. When trying to run old scripts
using new software versions, you may run into compatibility problems:
the code may not run or, worse, it will run but produce different results.
For this reason, it's a good idea to keep track of the versions of R
and any packages you used. To this end, you can include the command
`sessionInfo()` at the bottom of your scripts. 
At the bottom of this page, I've used a different command that serves
the same purpose but that requires another add-on package to be installed.

If you should ever observe that your code doesn't run as expected,
you can now check if differences in software versions could be to blame.
If so, you could try installing the old software versions to so if this solves
the problem. [[Using different R versions](https://support.posit.co/hc/en-us/articles/200486138-Changing-R-versions-for-the-RStudio-Desktop-IDE); [Installing old package versions](https://support.posit.co/hc/en-us/articles/219949047-Installing-older-versions-of-packages)]
:::
...
2. _Without running these commands_, predict what their output will look like:
```{r, eval = FALSE}
...
under 'Arguments'. Predict what the output of the following two commands will look like,
...
For instance, the `stringr` package, which is loaded automatically as part
of the `tidyverse`, contains the function `str_detect()`. As its name
suggests, this function can be used to detect if a string contains a 
...
Related functions are `str_starts()` and `str_ends()`
for checking if a string starts or ends with a particular combination of
symbols, as well as `str_sub()` for extracting substrings based on their position
in the string. As the examples below illustrate, these queries are case-sensitive:
...
regular expressions, which we'll touch on later. For more information
about the `str_*()` functions, see the [stringr documentation](https://stringr.tidyverse.org/).
...
skills |> 
  ...
  summarise(mean_narr = mean(Narration, na.rm = TRUE),
            mean_arg  = mean(Argumentation, na.rm = TRUE),
            .groups = "drop") |> 
  summarise(median_mean_narr = median(mean_narr))
...
hodges_lehmann <- function(x, na.rm = FALSE) {
  ...
  median(pairwise_means[upper.tri(pairwise_means, diag = FALSE)]) # median of means of x_i, x_j with i < j
}
```
...
using a grid [@Lambelet_HELASCOT_writing]; 
...
using a 1-to-9 scale [@Vanhove2019].
...
see @Vanhove_lexrich_TR for details.
...
add columns called `Language`, `Time`, `TextType` and `Criterion` (uppercase 'c')
to the `writing_long` tibble. Evidently, these columns should contain
the appropriate pieces of information. Then drop the `criterion` (lowercase 'c')
...
3. Combine both files in such a way that, for each text, you have both the
   ...
   (Hint: You can't directly use one of the `*_join()` functions. 
...
# Suggested reading
The Bible for working with tibbles in R is _R for Data Science_ [@Wickham2023], 
