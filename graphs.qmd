...
but a **density estimate**. To this end, the y-axis is scaled in such a way
...
of what they are now. This is because the area covered by the histogram
needs to equal 1.
...
d_hist |> 
  ...
  stat_function(fun = pnorm, 
                args = list(mean = mean(d_hist$Wortschatz),
                      sd = sd(d_hist$Wortschatz)),
                linetype = "dashed") +
  xlab("Wortschatz") +
  ylab("Empirical cumulative probability")
```
...
```{r}
d_hist |> 
  ggplot(aes(sample = Wortschatz)) +
  geom_qq(shape = 1) +
  geom_qq_line() +
  xlab("Standard normal quantiles") +
  ylab("Empirical quantiles of Wortschatz")
```
...
Pick $a, b$ such that $\sum_{i=1}^n (y_i - a - bx_i)^2$ is minimal.
