...
I strongly advise you to do so in your own
...
# Preliminaries
Refer to the section _Working with R and RStudio_ in the datasets primer.
...
A histogram shows how the observations from a single numeric variable are distributed ('univariate distribution').
...
We use `aes()` command (short for _aesthetics_) on the second line to specify which
aspect of the data should be mapped onto which aspect of the plot.
For a bare-bones histogram, we only need to specify that we want to plot the values of the variable
of our choice along the x-axis.
If we only executed the first two lines of the code below, all we'd see is a blank canvas, though.
...
::: {.callout-caution}
## The shape of a histogram doesn't depend on _just_ the bin width
The code belows draws two histograms for the `Englisch` data.
The difference between them is that, for the first one, the bins used are
$(0.45, 0.5], (0.5, 0.55], \dots, (0.85, 0.9]$, whereas for the second one,
they are $(0.475, 0.525], (0.525, 0.575], \dots, (0.875, 0.925]$.
In both cases, though, the width of the bins is 0.05 points.
Still, the histograms look somewhat different.

Incidentally, plots drawn using `ggplot()` can be stored as objects (here `h1` and
`h2`). You can combine these plots into a single figure using the `patchwork` package.
```{r, fig.width = 8, fig.height = 3.5}
h1 <- ggplot(data = d_hist,
                 aes(x = Englisch,
                     y = after_stat(density))) +
  geom_histogram(breaks = seq(0.45, 0.9, 0.05),          
                 fill = "lightgrey",
                 colour = "black") +
  xlab("English vocabulary score") +
  ylab("density")
h2 <- ggplot(data = d_hist,
                 aes(x = Englisch,
                     y = after_stat(density))) +
  geom_histogram(breaks = seq(0.45, 0.9, 0.05) + 0.025,          
                 fill = "lightgrey",
                 colour = "black") +
  xlab("English vocabulary score") +
  ylab("density")
library(patchwork)
h1 + h2 # this requires patchwork to work
```
...
But see the explanation at `?quantile` for details.)
:::
...
The reason is that several participants in the same condition obtained the same result, 
...
To illustrate this, we use data from an experiment by @Berthele2011b.
...
to make out the overlapping data points better and changed the German axis
names and labels to English ones.
```{r}
...
ggplot(berthele,
  ...
  geom_pointrange(data = potenzial_summary,
                  aes(y = mean_Potenzial, 
                      ymin = mean_Potenzial - sd_Potenzial,
                      ymax = mean_Potenzial + sd_Potenzial),
                  size = 1, colour = "blue", shape = 8) +
  facet_grid(cols = vars(Name)) +
  scale_x_discrete(name = "Code-switching condition",
                   limits = c("ohne", "mit"),         # specifies in what order the x-values should appear, referencing their actual names
                   labels = c("without", "with")) +   # specifies how these x-values should be *called*
  ylab("Academic potential rating")
```
...
::: {.callout-caution}
## Plot parameters inside and outside of `aes()`
In the plot above, we put the `colour` and `linetype` parameter
inside the `aes()` call. As a result, different colours are used
for different values of `Language` and different linetypes are used
for different values of `CourseType`. It is also possible to set
such parameters outside of the `aes()` call if you want to specify 
fixed colours, linetypes or what-have-you. For instance,
the following plot uses different `shape`s and `linetype`s to
distinguish between languages and course types (admittedly the
shapes are difficult to differentiate), whereas
the `colour` of the lines and of the points are set to 
`darkgreen` and `darkorange`, respectively:
...
Notice what happens when we 'set' the `colour` parameter to `"darkgreen"`
in the `aes()` call instead:
...
ggplot(data = slavin,
  ...
  geom_point()
```
Effectively, we've asked `ggplot2` to vary the colour of the lines
and points _depending on_ the value `"darkgreen"` -- which is a constant.
We did _not_ set the colour _to_ `"darkgreen"`.
:::

::: {.callout-tip}
## Overriding default aesthetic mappings
To override the default colour, linetype, shape etc. mappings,
you can use additional layers such as `scale_colour_manual()`,
`scale_shape_manual()` etc. The example below illustrates the principle;
refer to the [documentation](https://ggplot2-book.org/scales-other) for more details.
```{r}
ggplot(data = slavin,
       aes(x = Grade, 
           y = Mean,
...
```
Colours can also be specified using hexadecimal (HEX) RGB notation.
On the [ColorBrewer 2.0 website](https://colorbrewer2.org/),
you can preview colour combination that work well together
and obtain their HEX codes.
:::
...
When facetting along multiple dimensions,
we're faced with the decision as to which variable we should facet by 
horizontally and which variable we should facet by vertically.
A useful guiding principle is that comparing values plotted on a shared
axis is easier than comparing values plotted on two different axes.
For instance, in the plot where we split up the line chart vertically by programme (`TBE` vs. `EI`)
and horizontally by language (`English`, `Spanish`),
it is very easy to compare the English and Spanish values
for the EI programme with each other as well as to compare
the English and Spanish values for the TBE programme with
each other: Within each programme, the values are plotted
along a common y-axis. 
However, comparing, say, the English values in the EI programme to 
those in the TBE programme requires more effort, as these values are plotted along different y-axes.
So, in this particular case, if you need to facet both vertically and horizontally,
you'd facet horizontally by the variable related to the most important comparisons
and vertically by the variable related to any further comparisons.
That said, in this example, we can easily present the data clearly without facetting
or using just horizontal facetting.

In the first chapter of his freely available book,
@Healy2019 discusses how the lay-out of a graph affects the ease with which such visual decoding
...
The data for this walkthrough stem from @Lambelet_HELASCOT_writing 
and @Pestana_HELASCOT_reading.
We want to visualise the results on the Portuguese reading and writing (both argumentative and narrative) tasks
between three groups: Portuguese children in Portugal, children with a Portuguese background
in French-speaking Switzerland, and children with a Portuguese background in German-speaking Switzerland.
Ideally, we should also be able to make out trends across the three times of data collection.
...
fairly basic graph, found some problem with it, tried to fix it, and so on. 
The end result is a plot with 27 boxplots 
that hopefully enables readers to compare the three language groups
...
convert them to a longer format (with the three skills underneath rather than beside each other).
Then only retain the data for Portuguese tests, and get rid of the `NA` scores.
```{r}
skills <- read_csv(here("data", "helascot_skills.csv"))
skills_longer_portuguese <- skills |> 
  pivot_longer(Reading:Narration, values_to = "score", names_to = "skill") |> 
  filter(LanguageTested == "Portuguese") |> 
  filter(!is.na(score))
```
Inspect the structure of the data by typing its name at the prompt. (Not shown here.)

We'll need to extract some information from the `Subject` strings.
(See the section on **String manipulation** in the datasets tutorial.)

```{r}
skills_longer_portuguese <- skills_longer_portuguese |> 
  mutate(
...
Our default for plotting more or less fine-grained data is the boxplot.
We'll plot different boxplots per `language_group`.
We can use facetting to draw separate smaller plots for different times and different tasks.
```{r}
ggplot(skills_longer_portuguese,
       aes(x = language_group, y = score)) +
  geom_boxplot() +
  facet_grid(cols = vars(Time), rows = vars(skill))
```

This graph has a few problems. The most pressing two are that 
the group labels are illegible and that the scores on the different tasks
are plotted on the same scale. The latter results in very compressed boxplots
for the reading data and can be fixed by specifying that that the y-axis should be scaled freely:
...
To fix the first problem, there are a few options.
We could shorten the labels to, say, `FR-PT`, `GE-PT` and `mono PT` or so.
We could also write the labels vertically or diagonally rather than horizontally.
My preferred solution, though, is to _flip the axes_ and draw the boxplots themselves
horizontally rather than vertically. Like so:

```{r}
ggplot(skills_longer_portuguese,
       aes(y = language_group, x = score)) +
  geom_boxplot() +
  facet_grid(cols = vars(Time), rows = vars(skill),
             scales = "free_x")
```

Note that the trick with `scales = free_x` doesn't quite work here since there
are scores for all three tasks in each column.
We can sidestep this problem by changing the way the graph is facetted:

```{r}
ggplot(skills_longer_portuguese,
       aes(y = language_group, x = score)) +
  geom_boxplot() +
  facet_grid(rows = vars(Time), cols = vars(skill),
             scales = "free_x")
```
This isn't bad. But we can add some more structure to this graph.
Specifically, it was expected that French-Portuguese children would do better
on some of the Portuguese tasks than German-Portuguese children due to the
linguistic proximity.
We can override the order in which the groups are plotted using `scale_y_discrete()`.
While we're at it, let's suppress the axis name for the y axis, too.

```{r}
ggplot(skills_longer_portuguese,
       aes(y = language_group, x = score)) +
  geom_boxplot() +
  scale_y_discrete(limits = c("German-Portuguese", "French-Portuguese", "Portuguese control"),
                   name = element_blank()) +
  xlab("Test score") +
  facet_grid(rows = vars(Time), cols = vars(skill),
             scales = "free_x")
...
To give readers a better sense of the distributions underlying these boxplots,
let's add the individual data points to this plot.
Don't forget to set `outlier.shape = NA` in the `geom_boxplot()` call.
Play with the `height` parameter to find one you find suitable.

```{r}
ggplot(skills_longer_portuguese,
       aes(y = language_group, x = score)) +
  geom_boxplot(outlier.shape = NA) +
  geom_point(position = position_jitter(width = 0, height = 0.1), shape = 1) +
  scale_y_discrete(limits = c("German-Portuguese", "French-Portuguese", "Portuguese control"),
                   name = element_blank()) +
  xlab("Test score") +
  facet_grid(rows = vars(Time), cols = vars(skill),
...
Hm, that looks a bit ugly - the circles are a bit _too_ present.
We can increase their transparency, though.
And finally, let's override the default grey plotting theme.

```{r}
ggplot(skills_longer_portuguese,
       aes(y = language_group, x = score)) +
  geom_boxplot(outlier.shape = NA) +
  geom_point(position = position_jitter(width = 0, height = 0.1), shape = 1, alpha = 1/4) +
  scale_y_discrete(limits = c("German-Portuguese", "French-Portuguese", "Portuguese control"),
                   name = element_blank()) +
  xlab("Test score") +
  facet_grid(rows = vars(Time), cols = vars(skill),
...
Having drawn a graph, it's a good idea to reflect on what comparisons
can easily be made based on this graph and which ones are difficult to make.
You want to ensure that the comparisons relevant for the primary research questions
are easy to make.
The plot above makes it very easy to compare the scores on a particular
task between the different groups, as these can be read off a shared
axis (the x-axis) (comparison within a given panel).
It's also pretty easy to see that, for a given group and a given task,
the scores at the second time are higher than those at the first time,
and those at the third time are typically higher than those at the second time
(comparison across panels in the same column).
If the temporal component is more important to you than the group comparison proper,
use `language_group` as a facetting variable and `Time` as the `y` variable.
This way, you ensure that the answer to the primary research question can be gleaned
easily from the graph.

What's difficult to do is to compare the scores between different columns -- 
but it wouldn't make much sense to do so as these scores represents different variables!
...
presented in a bar chart or in a Cleveland dot plot [@Cleveland1984].

The graph in the middle is a **bar chart**, the one of the right
a (Cleveland) **dot plot**. Both are better choices than pie charts.
...
Visualise these aggregated data, for instance using boxplots. 
...
```{r, echo = FALSE}
x_vals <- seq(0, 1.3, length.out = 1000)
y_vals <- pmax((1 - (x_vals/1)^3)^3, 0)
df <- data.frame(x = x_vals, y = y_vals)
ggplot(df, aes(x, y)) +
  geom_line(color = "black", linewidth = 1) +
  xlab(expression(paste("distance of ", x[i], " from x*"))) +
  scale_x_continuous(breaks = c(0, 0.5, 1),
                     labels = c("0", "M/2", "M")) +
  ylab(expression(w[i])) +
  labs(title = "Weighting function used in LOESS") +
  theme_minimal()
```
...
By default, quadratic regression is used, which means that 
...
The `loess()` function, which `geom_smooth()` draws on, 
implements a few improvements on this procedure.
:::
...
Scatterplots can also be useful
...
::: {#exr-vandenbroek1}
## Drawing a Tukey mean--difference plot
@VanDenBroek2018 investigated whether repeated exposures to foreign-language
words enhanced word retention more if these words were embedded in an information-rich
context that allows the learners to infer the meaning of the words from context
or if these words were embedded in an information-poor context that forces
learners to retrieve the meaning of these words from memory.
We will focus on their Experiment 2, which was a within-subjects experiment, 
the summary data for which are reported in their Table 2 (p. 565).
Specifically, we will focus on the Retrieval and Context inference
conditions (ignoring the Retrieval-plus-context inference condition) and on the productive tests (i.e., translation from the L1 to the L2) at the delayed posttest.
@VanDenBroek2018 report that, on average, the participants correctly translated 42% of the words
in the Retrieval condition (with a standard deviation of 21 percentage points)
and 37% in the Context inference condition (also with a standard deviation of 21 percentage points).
...
2. Create a new tibble that contains the proportion of correctly translated Dutch words at the delayed posttest per participant and per condition. Do not include the `retrievalc` condition.
3. Check if the average proportion per condition matches the numbers given in Van den Broek et al.'s Table 2.
4. Create a tibble that contains, for each participant, the average proportion of correctly translated words over the two conditions as well as the difference between these proportions (i.e., the proportion of correct translations in the `context` condition minus the proportion of correct translations in the `retrievalf` condition).
5. Draw a scatterplot of the tibble you constructed in the previous step, with the averages along the x-axis and the differences along the y-axis. Add a horizontal line at $y = 0$ to the plot (use `geom_hline(yintercept = 0, linetype = "dashed")`). Also add a scatterplot smoother.
6. Interpret the plot.
:::

::: {#exr-vandenbroek2}
## The same results as a Cleveland dotplot
Using the same data as in the previous exercise,
draw a Cleveland dotplot showing the proportion of correctly translated
words in both conditions for each participant. Which visualisation do you prefer
for these data?
:::
...
You can only log-transform a variable if all its values are strictly positive.
Sometimes, when zeroes occur, the square-root transformation is useful (i.e.,
replace the values with their square roots).
Other transformations exist (Google _Box-Cox transformation_ and _Yeo-Johnson transformation_)
but come at the price of reduced interpretability.
A special use-case that may be of interest to language researchers concerns
the treatment of corpus frequencies; see @Brysbaert2013b for guidance.
...
::: {#exr-logarithms}
## Logarithms
Draw the scatterplot matrix again,
but this time, use base 10 when log-transforming
the number of tokens. (Use `log10()`. If
the log-10-transformed value of $x$ is $y$,
then $10^y = x$.) What changes and what stays the same?
Draw the scatterplot once more,
using the natural logarithm for the log-transformation.
(Use `log()`. The natural logarithm is the logarithm
that uses Euler's number $e = 2.71828\dots$ as its base.)
What changes and what stays the same?
:::

**Update (2025/05/26):** Part of what changes is the shape of the histograms.
The shape of the underlying distribution, up to scaling, doesn't change, though.
The shape of the histogram changes because, by default, the boundaries
of the histogram bins are set to 'pretty' values (see `?pretty`).

# Suggested reading
Kieran Healy's _Data visualization: A practical introduction_ [@Healy2019] does a stirling
...
The graphs we've covered are static.
For interactive graphs, check out [`ggplotly()`](https://plotly.com/ggplot2/)
and [`shiny`](https://shiny.posit.co/).
